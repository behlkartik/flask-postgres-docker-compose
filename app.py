...
INSERT_MOVIE_RETURN_ID = "INSERT INTO movie(name, duration, rating) VALUES(%s, %s, %s) RETURNING id"
INSERT_CAST_RETURN_ID = "INSERT INTO casts(actor_name, movie_id) VALUES(%s, %s) RETURNING id"
GET_MOVIE_WITH_ID = "SELECT * FROM movie WHERE id=(%s)"
GET_ALL_MOVIES = "SELECT * FROM movie"
GET_MOVIE_WITH_CAST = "SELECT * FROM movie JOIN casts ON movie.id = casts.movie_id WHERE movie.id=(%s)"
...
connection = psycopg2.connect(dbname=db_name, user=user, password=password, host=db_host)

def get_list_of_dict_from_cursor(cursor, type):
    if type == "one":
        movies = [dict((cursor.description[i][0], value) \
...
def get_dict_from_cursor(cursor):
    movie = dict((cursor.description[i][0], value) for i, value in enumerate(cursor.fetchone()))
    return movie
...
def create_database():
    with connection:
        with connection.cursor() as cursor:
            cursor.execute(f"SELECT 'CREATE DATABASE {db_name}' \
...
def create_movie():
    ...
    movie_name, movie_duration, movie_rating = request_data["name"], request_data["duration"], request_data["rating"]
    with connection:
        with connection.cursor() as cursor:
            cursor.execute(INSERT_MOVIE_RETURN_ID, (movie_name, movie_duration, movie_rating))
